...
### Подбор пароля для пользователя "ручками"

   Здесь приведён пример подбора пароля для пользователя Ubuntu.

    
    #!/usr/bin/bash
    login=$(whoami)
    echo "$login"
    var=$(sudo cat /etc/shadow | grep "$login" | cut -d: -f2)
    func=$(echo "$var" | cut -d$ -f2)
    solt=$(echo "$var" | cut -d$ -f3)
    hash=$(echo "$var" | cut -d$ -f4)
    var2=$(mkpasswd -m SHA-512 -S "$solt" "$1")
    hash2=$(echo "$var2" | cut -d$ -f4)
    [[ "$hash" == "$hash2" ]] && echo "Пароль верный" || echo "Пароль неверный"
   ...
   Соответственно этой строкой мы получаем нужную информацию о нашем пользователе, которая хранится в /etc/shadow о нашем текущем пользователе и записываем функцию,которой хэшировался наш пароль,соль нашего пароля, и хэш с которым мы будем в конце сравнивать полученный результат.
    
    
    
    var2=$(mkpasswd -m SHA-512 -S "$solt" "$1")
    
   Этой строкой мы хэшируем наш пароль данный как аргумент нашему скрипту с солью взятой из /etc/shadow функцией SHA-512.
   Далее мы можем увидеть интерпретаторы для популярных функций хэширования, по которым мы можем определить каким методом хэшировался пароль.
...
* $6 для SHA- 512
   ...
   ```bash
    var2=$(mkpasswd -m SHA-512 -S "$solt" "$1")
   ```
Переменной var2 присваиваем распарсенный хэш нашего прелполагемого пароля, далее сравниваем его с действительным.
Попробуйте проделать всё самостоятельно!
...
1.Изучить понятие радужные таблицы, таблицы поиска и обратные таблицы поиска.
1. Познакомиться с утилитой fakeroot(  Возможно ли  повторить задание данного ПЗ из под fakeroot? )
